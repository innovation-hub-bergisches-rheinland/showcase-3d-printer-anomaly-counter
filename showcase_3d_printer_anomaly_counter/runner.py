    ...
    for msg in kafka_pc.consumer:
        try:
            ...
            if msg.topic() == kafka_pc.config["RAW_DATA_TOPIC"]:
                ...
                if data["tags"]["TEMPERATURE_SOURCE_TAG_KEY"] == kafka_pc.config["TEMPERATURE_SOURCE_TAG_VALUE"]:
                    if printing == False and data["fields"][kafka_pc.config["SET_TEMPERATURE_FIELD_KEY"]] > 0:
                        printing = True
                        request = Request(data["fields"]["time"])
                        send_message(producer, kafka_pc, data=request.toJSON()) 
                    elif printing == True and data["fields"][kafka_pc.config["SET_TEMPERATURE_FIELD_KEY"]] == 0:
                        printing = False
                        request.stop_request(data["fields"]["time"])
                        send_message(producer, kafka_pc, data=request.toJSON())
            elif msg.topic() == kafka_pc.config["ANOMALY_TOPIC"] and printing == True:
